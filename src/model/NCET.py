...
from model.layers.CustomRNN.cells import StateGraphGRUCell, RNNCell
...
class NCETModel(nn.Module):
    def __init__(self, config):
        ...
        # entity state
        self.state_lstm = LSTMEncoder(self.hidden_dim*4, self.hidden_dim, self.batch_size, self.max_sent_length)
        self.state_rnn = CustomRNN(cell_class=StateGraphGRUCell, input_dim=self.hidden_dim*4, hidden_dim=self.hidden_dim, batch_first=True, bidirectional=True)
        ...
        # entity location
        self.location_lstm = LSTMEncoder(self.hidden_dim*4, self.hidden_dim, self.batch_size, self.max_sent_length)
    ...
    def forward(self, words, words_idxs, verbs, words_length, sents_length, verbs_idxs_sents, entity_idxs_sents, entity_to_idx, idx_to_entity, location_candidate_idxs_sents, location_candidate_to_idx, idx_to_location_candidate):
        ...
        if self.use_state_graph:
            ...
            entity_states_rnn_encoding, _ = self.state_rnn(entity_states_input) # [batch_size, seq_len, entity_size, hidden_dim*2]
            ...
            for entity, entity_idxs in entity_idxs_sents.items():
                ...
                for i, idxs in enumerate(entity_idxs):
                    if not idxs:
                        ...
                        entity_encoding = word_lstm_encoding[idxs[0].item():idxs[1].item()+1]
                        entity_encoding = torch.mean(entity_encoding, dim=0).view(-1)
                        entity_encoding = torch.cat([entity_encoding, verbs_sents[i]], dim=-1)
                    entity_sents.append(entity_encoding)
                start_encoding = torch.zeros(entity_encoding.size()).to(device)
                end_encoding = torch.zeros(entity_encoding.size()).to(device)
                entity_sents = [start_encoding] + entity_sents + [end_encoding]
                entity_sents = torch.stack(entity_sents).unsqueeze(dim=0)
                states_lstm_encoding = self.state_lstm(entity_sents, [sents_length])[:, :sents_length, :]
        ...
        for entity, entity_idxs in entity_idxs_sents.items():
            ...
            for i, idxs in enumerate(entity_idxs):
                if not idxs:
                    entity_encoding = torch.zeros(self.hidden_dim*2).to(device)
                else:
                    entity_encoding = word_lstm_encoding[idxs[0].item():idxs[1].item()+1]
                    entity_encoding = torch.mean(entity_encoding, dim=0).view(-1)
                entity_sents.append(entity_encoding)
            ...
            end_encoding = torch.zeros(entity_encoding.size()).to(device)
            entity_sents = [start_encoding] + entity_sents + [end_encoding]
            entity_sents = torch.stack(entity_sents).unsqueeze(dim=0)   # [batch_size, sents_len, hidden_dim * 2]
            ...
            for location_candidate, location_candidate_idxs in location_candidate_idxs_sents.items():
                ...
                for i, idxs in enumerate(location_candidate_idxs):
                    if not idxs:
                        location_encoding = torch.zeros(self.hidden_dim*2).to(device)
                    else:
                ...
                location_sents = torch.stack(location_sents).unsqueeze(dim=0) # [batch_size, sents_len, hidde_dim * 2]
                # concate entity & locaation
                entity_location_sents = torch.cat([entity_sents, location_sents], dim=-1) # [batch_size, sents_len, hidden_dim * 4]
                location_lstm_encoding = self.location_lstm(entity_location_sents, [sents_length])[:, :sents_length, :]
                location_logit = self.location_mlp(location_lstm_encoding)
                location_logits[location_candidate_to_idx[location_candidate]] = location_logit
